...
static int8_t USBFS_Control  (uint8_t cmd, uint8_t* pbuf, uint16_t length);
static int8_t USBFS_Receive(uint8_t* pbuf, uint32_t *Len);

USBD_CDC_ItfTypeDef USBD_CDC_CanAnalyzer_fops = 
{
  USBFS_Init,
  USBFS_DeInit,
  USBFS_Control,
...
static uint8_t USB_Buffer[CDC_DATA_FS_OUT_PACKET_SIZE];
...
/**
  ...
  * @retval Result of the operation: USBD_OK if all operations are OK else USBD_FAIL
  */
static int8_t USBFS_Receive(uint8_t* Buf, uint32_t *Len)
{
	USBPacketData data;
	data.Size = *Len;
